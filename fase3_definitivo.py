...
def parse_matches(text, section_header):
    ...
    for i, (pos, date, comp, result) in enumerate(positions):
        ...
        hilal_home = "Hilal" in home_team
        
        stats['nassr_goals'] = hg if nassr_home else ag
        stats['hilal_goals'] = hg if hilal_home else ag
        
        if all_section:
            metrics = [
                ('Total shots', 'total_shots', 'nassr_shots', 'hilal_shots'),
                ('Corner kicks', 'total_corners', 'nassr_corners', 'hilal_corners'),
                ('Yellow cards', 'total_yellows', 'nassr_yellows', 'hilal_yellows'),
                ('Fouls', 'total_fouls', 'nassr_fouls', 'hilal_fouls'),
                ('Offsides', 'total_offsides', 'nassr_offsides', 'hilal_offsides'),
                ('Shots on target', 'total_sot', 'nassr_sot', 'hilal_sot'),
                ('Goalkeeper saves', 'total_saves', 'nassr_saves', 'hilal_saves')
            ]
            ...
            for label, t_key, n_key, h_key in metrics:
                ...
                if l is not None and r is not None:
                    stats[t_key] = l + r
                    # Para el propio equipo, extraemos su estadística específica
                    if "Nassr" in section_header:
                        stats[n_key] = l if nassr_home else r
                    elif "Hilal" in section_header:
                        stats[h_key] = l if hilal_home else r
                    elif "H2H" in section_header:
                        stats[n_key] = l if nassr_home else r
                        stats[h_key] = r if nassr_home else l
...
nassr_last_10 = parse_matches(text, "Ultimos 10 - Al-Nassr")
hilal_last_10 = parse_matches(text, "Ultimos 10 - Al-Hilal")
...
for mg in all_groups:
    ...
    for mid in mg.get('marketIds', []):
        ...
        is_nassr = 'nassr' in mname
        is_hilal = 'hilal' in mname
        ...
        if 'esquina' in mname or 'corners' in mname:
            ...
            else: stat_key = 'nassr_corners' if is_nassr else 'hilal_corners' if is_hilal else 'total_corners'
        elif 'tarjetas' in mname or 'cards' in mname:
            if is_first_half: stat_key = 'first_half_yellows'
            else: stat_key = 'nassr_yellows' if is_nassr else 'hilal_yellows' if is_hilal else 'total_yellows'
            ...
            if 'esquina' not in mname:
                stat_key = 'nassr_shots' if is_nassr else 'hilal_shots' if is_hilal else 'total_shots'
        elif 'faltas' in mname:
            stat_key = 'nassr_fouls' if is_nassr else 'hilal_fouls' if is_hilal else 'total_fouls'
        elif 'fueras' in mname:
            stat_key = 'nassr_offsides' if is_nassr else 'hilal_offsides' if is_hilal else 'total_offsides'
        elif 'paradas' in mname:
            stat_key = 'nassr_saves' if is_nassr else 'hilal_saves' if is_hilal else 'total_saves'
        elif 'goles' in mname or 'goals' in mname:
            if not is_first_half and 'mitad' not in mname:
                stat_key = 'nassr_goals' if is_nassr else 'hilal_goals' if is_hilal else 'total_goals'
        ...
        for o in odds:
            ...
            if 'nassr' in stat_key:
                matches_own = nassr_last_10
            elif 'hilal' in stat_key:
                matches_own = hilal_last_10
            else:
                matches_own = nassr_last_10 + hilal_last_10
        ...
        if 'ambos' in mname and 'marcan' in mname and 'mitad' not in mname:
            ...
            for o in odds:
                ...
                if 'sí' in o['name'].lower() or 'si' in o['name'].lower():
                    ...
                    if uid in seen: continue
                    seen.add(uid)
                    own = [1 if x['btts']==1 else 0 for x in nassr_last_10 + hilal_last_10]
...
for c in candidates:
    r = c['res']
    if r['total_pr'] >= 0.80 and r['h2h_pr'] >= 0.70 and r['own_pr'] >= 0.80 and c['price'] <= 1.70:
...
print("🏆 LADRILLOS DE ORO FILTRADOS (H2H >= 70%, Propios >= 80%, Cuota <= 1.70)")
for b in gold_bricks[:15]:
    r = b['res']
    print(f"[{b['group']}] {b['market']} | {b['sel']} @ {b['price']}")
    print(f"   -> Propios: {r['own_wins']}/{r['own_total']} ({r['own_pr']*100:.1f}%) | H2H: {r['h2h_wins']}/{r['h2h_total']} ({r['h2h_pr']*100:.1f}%) | Total: {r['total_wins']}/{r['total_games']} ({r['total_pr']*100:.1f}%)\n")

print("\n🚀 VALUE BETS (H2H >= 60%, Propios >= 60%, Cuota >= 1.60)")
for v in value_bets[:15]:
    r = v['res']
    print(f"[{v['group']}] {v['market']} | {v['sel']} @ {v['price']} (Value: {v['value']*100:.1f}%)")
    print(f"   -> Propios: {r['own_wins']}/{r['own_total']} ({r['own_pr']*100:.1f}%) | H2H: {r['h2h_wins']}/{r['h2h_total']} ({r['h2h_pr']*100:.1f}%) | Total: {r['total_wins']}/{r['total_games']} ({r['total_pr']*100:.1f}%)\n")
